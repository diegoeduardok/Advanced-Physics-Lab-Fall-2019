...
with open("C1t2mgh2ogood00000.txt", 'r') as file:
	for line in file.readlines()[1240000:1850000:10]: #Drop data points before and after useful part. Downsample by 10.
...
peaks, properties = find_peaks(amplitudes, height=exponential(times, 5.65, 1/0.04, 0.3), distance=500)
...
plt.title("MG spin-echo measurement for distilled water")
plt.xlabel("Time (s)")
plt.ylabel("Amplitude (V)")

plt.plot(times, amplitudes)
plt.scatter(times[peaks], amplitudes[peaks], s=20, marker="x", c="r")
plt.plot(np.arange(0, 2.4, 0.001), exponential(np.arange(0, 2.4, 0.001), *fit_params))
plt.legend(["Raw data", 
			"Fit: " + "y={0:.2f}".format(fit_params[0]) + '*E^(-x/' + "{0:.2f}".format(1/fit_params[1]) + ") + " + "{0:.2f}".format(fit_params[2]),  
			"Filtered peaks"])

plt.savefig("T2_distilled_water_MG.png")

with open("T2_distilled_water_MG.txt", 'w') as file:

	file.write("MG spin-echo measurement for distilled water.\n")
